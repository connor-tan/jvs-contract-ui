import { sleep, type NetworkProvider, type UIProvider} from '@ton/blueprint';
import { Address, beginCell, Builder, Cell, Dictionary, type DictionaryValue, Slice } from "@ton/core";
...
export const getAccountLastTx = async (provider: NetworkProvider, address: Address) => {
    ...
    let res = null;
    if(api instanceof TonClient4){
        res = await api.getAccountLite(await getLastBlock(provider), address)
    }
    if(res == null || res.account.last == null)
...
export const waitForTransaction = async (provider:NetworkProvider, address:Address, curTx:string | null, maxRetry:number, interval:number=1000) => {
    ...
    do {
        ...
        let curState = null;
        if(api instanceof TonClient4){
            curState = await api.getAccountLite(lastBlock, address);
        }
        if(curState != null && curState.account.last !== null){
